...
def test_create_api_key_without_queries():
    header = {"x-api-key": "1.very_secret_key_100_real"}
    response = client.post(f"/api/v2/create-api-key/", headers=header)
    assert response.status_code == 422


def test_create_api_key_without_api_key():
    response = client.post(f"/api/v2/create-api-key/?new_key=maybe-iam-a-api-key&new_key_permissions=rwa")
